...
for i=1:num_instance
    temp=test_target(:,i);
    if((sum(temp)~=num_class) & (sum(temp)~=0))%%不全为0或全为1
...
for i=1:num_instance
    temp=test_target(:,i);
    Label_size(1,i)=sum(temp==ones(num_class,1));%每一列等于1的个数
    for j=1:num_class
        if(temp(j)==1)
            Label{i,1}=[Label{i,1},j];%% Label{i,1}记录test_target(:,i)等于1的下标
        else
            not_Label{i,1}=[not_Label{i,1},j];%%not_Label{i,1}记录test_target(:,i)等于0的下标
        end
    end
end
...
for i=1:num_instance
    temp=0;
    div = Label_size(i) * (num_class-Label_size(i));%%第i个样本中等于1的个数乘以等于0的个数
    ...
    temp_rankloss=temp_rankloss+temp/div;
end

RankingLoss=temp_rankloss/num_instance;
